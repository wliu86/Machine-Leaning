...
#After the check, we decided to keep the majority 20 categories for each 
#variable that has larger than 20 level, keep 24 for our "hour" variable,
#and keep the variables that have larger than 20 levels their original level number.
...
#As the grid plot shows, the best perfoems we receive from 
#Lasso Regression is at around 0.43.
...
#We tried to use random forest, but the result is worse than 
#the prsvious logistic regression (lasso and ridge model), so 
#we decided to use another ensemble method: boosting. It is 
#illustrated in our previous reading material. But it is a 
#relatively new method, and it normally performs better than 
#ramdom forest, so we decided to try it.

#As shown, this model takes probably 30 minutes to build, 
#and the log-loss on our validation data is 0.379887, so this is 
#our best model. Furthermore, since we have only limitted time, 
#we didn't try to ensemble more than 3 classifer in our adaboost. 
#But we assume when we ensemble, for example, 5 classifers, we would 
#receive even better performance. Finally, we deicede to use our current 
#adaboost model to make the prediction in the test datat.
